...
class Triangles extends SubTag{
        ...
        Triangles(XMLElement triangles, HashMap<String,SubTag> sources)
        {
            ...
            {
                if (triangles_input.getAttribute("semantic").equals("VERTEX"))
                {
                    offsetVertex = Integer.parseInt(triangles_input.getAttribute("offset"));
                    vertexSource = (Source)sources.get(sources.get(triangles_input.getAttribute("source").substring(1)).getNextSource());
                }
                if (triangles_input.getAttribute("semantic").equals("TEXCOORD"))
                {
                    offsetTextures = Integer.parseInt(triangles_input.getAttribute("offset"));
                    textureSource = (Source)sources.get(sources.get(triangles_input.getAttribute("source").substring(1)).getNextSource());
                }
            }
     ...
     */
        ...
        {
            String s = "";
            s+="Triangles from Geometry "+triangles.getParent().getParent().getAttribute("id")+":\n";
