 ...
 */
   class Lines extends SubTag{
        ...
        Lines(XMLElement lines, HashMap<String,SubTag> sources)
        {
            ...
            {
                if (lines_input.getAttribute("semantic").equals("VERTEX"))
                {
                    offsetVertex = Integer.parseInt(lines_input.getAttribute("offset"));
                    vertexSource = (Source)sources.get(sources.get(lines_input.getAttribute("source").substring(1)).getNextSource());
                }
            }
     ...
     */
        ...
        {
            String s = "";
            s+="Lines from Geometry "+lines.getParent().getParent().getAttribute("id")+":\n";
