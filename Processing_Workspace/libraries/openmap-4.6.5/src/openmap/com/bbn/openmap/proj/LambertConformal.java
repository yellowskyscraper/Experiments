 ...
 * <br>
 * <b>NOTE: </b> This implementation only works for the northern hemisphere.
 * <br>
 * <br>
...
public class LambertConformal extends Proj {
     ...
     */
    protected LambertConformal(LatLonPoint center, float scale, int width,
            int height, float centralMeridian, float sp_one, float sp_two, Ellipsoid ellps) {
     ...
     * 
     * @param center
     *            center location for projections
     * @param scale
     *            scale of projection
     * @param width
     *            width of projection
     ...
     * @param sp_one
     *            Standard Parallel One in degrees.
     * @param sp_two
     *            Standard Parallel Two in degrees.
     * @param reference_latitude
     *            the latitude for the origin of the projection
     * @param falseEasting
     *            number of meters added as buffer to origin E/W.
     * @param falseNorthing
     *            number of meters added as buffer to origin for N/S.
     * @param ellps
     *            the {@link Ellipsoid} used for the projection
     */
    public LambertConformal(LatLonPoint center, float scale, int width,
            ...
            double reference_latitude, double falseEasting, double falseNorthing, Ellipsoid ellps) {
    ...
    public void computeParameters() {
        ...
        double m1 = Math.cos(phi1)/Math.pow(1-Math.pow(e*Math.sin(phi1),2.0),0.5);
        double m2 = Math.cos(phi2)/Math.pow(1-Math.pow(e*Math.sin(phi2),2.0),0.5);
    ...
    public LatLonPoint worldToLL(double x, double y, LatLonPoint llp) {
        ...
        double thetaR = Math.atan((x - falseEasting)/(rf - (y- falseNorthing)));
        double rR = Math.sqrt( Math.pow(x - falseEasting, 2) + Math.pow(rf - (y - falseNorthing),2));
        ...
        do {
            phiT1 = phiT2;
            phiT2 = (Math.PI/2 - 2 * Math.atan(tR * Math.pow((1.0-e*Math.sin(phiT1))/(1.0+e*Math.sin(phiT1)),e/2.0)));
    ...
    public LatLonPoint pixelToLL(int xabs, int yabs, LatLonPoint llp) {
        ...
        double x = locationCenterXLambert
                + (((double) xabs - locationCenterXPixel) / locationPixelsPerLambert);
        double y = locationCenterYLambert
                + ((locationCenterYPixel - (double) yabs) / locationPixelsPerLambert);
