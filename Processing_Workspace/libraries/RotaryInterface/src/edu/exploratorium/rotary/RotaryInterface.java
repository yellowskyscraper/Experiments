...
public class RotaryInterface {
	 ...
	 * {@value #ROTARY_EVENT_CALLBACK_NAME} callback method.
	 * 
	 * @param	p		Reference to the host PApplet.
	 * @param	serial	<tt>true</tt> to specify that the rotary encoder hardware
	 * 					will transmit values via a serial port.
	 * @param	eventHandlerScope	Scope for the {@value #ROTARY_EVENT_CALLBACK_NAME} callback method.
	 */
	 ...
	 * Default is {@value #DEFAULT_INPUT_CURVE}.
	 */
	 ...
	 * 
	 * Default is {@value #DEFAULT_STOP_THOLD}.
	 */
	 ...
	 * 
	 * Default is {@value #DEFAULT_ACCEL}.
	 */
	 ...
	 * Default is {@value #DEFAULT_INERTIA}.
	 */
	...
	void onRotaryInput (int value, int channel) {
		...
		if (value >= minCCW && value <= maxCCW) {
			...
			// value is out of range for this channel; ignore it.
	...
	private void init (PApplet p, boolean serial, Object eventHandlerScope) {
		...
		try {
			...
			System.err.println("No public method "+ methodName +"() found in class "+ C.getName() +".\n"+
	...
	private void updateSpeeds () {
		...
		for (int i=0; i<MAX_NUM_ROTARY_ENCODERS; i++) {
			...
			if (rotaryEventCallback != null) {
				try {
					rotaryEventCallback.invoke(rotaryEventCallbackScope, new Object[] { speeds[i], i });
				} catch (Exception e) {
