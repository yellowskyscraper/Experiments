...
/**
 * @invisible
 * GLTexturedPoint stores an (x, y) point and an array of associated texture coordinates (s[i], t[i]).
 */ 
public class GLTexturedPoint
{
    ...
    public GLTexturedPoint(XMLElement xml)
    {
        ...
        if (n <= 1) initTexCoordArrays(1);
        else initTexCoordArrays(n - 1);
        ...
        {  
            ...
            if (name.equals("coord"))
            {
                ...
                if (attr0.equals("x")) x = -1.0f;
                else x = child.getFloatAttribute("x");

                if (attr1.equals("y")) y = -1.0f;
                else y = child.getFloatAttribute("y");

                if (attr2 == null) dx = 0.0f;
                else if (attr2.equals("dx")) dx = -1.0f;
                else dx = child.getFloatAttribute("dx");

                if (attr3 == null) dy = 0.0f;
                else if (attr3.equals("dy")) dy = -1.0f;
                else dy = child.getFloatAttribute("dy");
            }
            else if (name.equals("texcoord"))
            {
                ...
                if (attr0.equals("s")) sVal = -1.0f;
                else sVal = child.getFloatAttribute("s");

                if (attr1.equals("t")) tVal = -1.0f;
                else tVal = child.getFloatAttribute("t");

                if (attr2 == null) dsVal = 0.0f;
                else if (attr2.equals("ds")) dsVal = -1.0f;
                else dsVal = child.getFloatAttribute("ds");

                if (attr3 == null) dtVal = 0.0f;
                else if (attr3.equals("dt")) dtVal = -1.0f;
                else dtVal = child.getFloatAttribute("dt");
