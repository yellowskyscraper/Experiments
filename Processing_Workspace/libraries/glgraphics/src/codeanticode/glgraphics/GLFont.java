...
public class GLFont extends PFont {
  public GLFont(PApplet parent, InputStream input) throws IOException {
    ...
    for (int i = 0; i < charCount; i++) {
      images[i] = new GLTexture(parent, twidth, theight, ALPHA);
      ...
      ((GLTexture)images[i]).loadTexture();
      //System.out.println();
    }
   ...
   */
  public GLFont(PApplet parent, Font font, boolean smooth, char charset[]) {
    ...
    images = new GLTexture[charCount];
    for (int i = 0; i < charCount; i++) {
      images[i] = new GLTexture(parent, mbox2, mbox2, ALPHA);
