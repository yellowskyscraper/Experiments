...
public class GLTexture extends PImage implements PConstants, GLConstants 
{
    ...
    public GLTexture(PApplet parent, int width, int height, GLTextureParameters params, int id)
    {
        super(width, height, params.format);  
        this.parent = parent;
     ...
     * @param float[] values  
     */
    public void filter(GLTextureFilter texFilter, GLTexture destTex, float[]...values)
    {
    ...
    /**
     * Applies filter texFilter using this texture as source, destTex as destination and destA as the
     * destination alpha value for the filter.
     * @param texFilter GLTextureFilter
     * @param destTex GLTexture				
     * @param destA float			
     */	
    ...
    {
        texFilter.apply(new GLTexture[] { this }, new GLTexture[] { destTex }, null, 1.0f, 1.0f, 1.0f, destA);
    }

    /**
     * Applies filter texFilter using this texture as source, destTex as destination and destA as the
     * destination alpha value for the filter. Sets all the value for all the parameters, by means of a 
     ...
     * @param float[] values  
     */	
    public void filter(GLTextureFilter texFilter, GLTexture destTex, float destA, float[]...values)
    {
        texFilter.setParameterValues(values);    	
        texFilter.apply(new GLTexture[] { this }, new GLTexture[] { destTex }, null, 1.0f, 1.0f, 1.0f, destA);
    }    
    ...
    /**
     * Applies filter texFilter using this texture as source, destTex as multiple destinations  and destA as the
     * destination alpha value constant for the filter.
     * @param texFilter GLTextureFilter
     * @param destTexArray GLTexture[]	
     * @param destA float			
     */	
    public void filter(GLTextureFilter texFilter, GLTexture[] destTexArray, float destA)
    {
        texFilter.apply(new GLTexture[] { this }, destTexArray, null, 1.0f, 1.0f, 1.0f, destA);
    }	
     ...
     * @param float[] values
     */	
    public void filter(GLTextureFilter texFilter, GLTexture[] destTexArray, float destA, float[]...values)
    {
        texFilter.setParameterValues(values);    	
        texFilter.apply(new GLTexture[] { this }, destTexArray, null, 1.0f, 1.0f, 1.0f, destA);
    }    
    ...
    * @param h float	 
    */    
    public void render(PGraphicsOpenGL renderer, float x, float y, float w, float h)
    {
    	render(renderer, x, y, w, h, 0, 0, 1, 1);
    }
    
    public void render(PGraphicsOpenGL renderer, float x, float y, float w, float h, float sx, float ty, float sw, float th)
    {
    	float fw, fh;
    ...
    public void putBuffer(int[] intArray, int format, int type)
    {
        ...
        if (format == ALPHA)
        {
          ...
          if (type == TEX_BYTE)
          {
              // Not sure why this works, because the integer values in intArray are converted to signed bytes, but the buffer is specified
              // as GL_UNSIGNED_BYTE.
              byte[] convArray2 = convertToAlpha(intArray);
              gl.glBindTexture(texTarget, tex[0]);
              gl.glTexSubImage2D(texTarget, 0, 0, 0, width, height, glFormat, GL.GL_UNSIGNED_BYTE, ByteBuffer.wrap(convArray2));
              gl.glBindTexture(texTarget, 0);
              return;              
          }
        }
        ...
        int glType;
        if (type == TEX_INT) glType = GL.GL_INT;
        else glType = GL.GL_UNSIGNED_BYTE;
    ...
    public void putBuffer(float[] floatArray, int nchan)
    {
        ...
        int glFormat;
        if (nchan == 1) glFormat = GL.GL_LUMINANCE;
        else if (nchan == 3) glFormat = GL.GL_RGB;
        else glFormat = GL.GL_RGBA;
    ...
    protected void initTexture(int w, int h)
    {
        ...
        gl.glTexParameteri(texTarget, GL.GL_TEXTURE_WRAP_S, GL.GL_CLAMP);
        gl.glTexParameteri(texTarget, GL.GL_TEXTURE_WRAP_T, GL.GL_CLAMP);
