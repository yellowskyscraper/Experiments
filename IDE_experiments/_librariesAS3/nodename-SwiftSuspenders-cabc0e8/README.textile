 ...
 * values
 * factory functions (which are invoked once for each injection; see https://gist.github.com/418821 for example usage)
...
SwiftSuspenders supports four types of dependency definitions:
* *value bindings*, which simply map an injection request to be satisfied by injecting the given object
* *factory bindings*, which map an injection request to be satisfied by injecting a new object returned by the given factory function
